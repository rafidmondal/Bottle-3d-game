...
import { FlipFeedback, FlipLandingOutcome, AiDifficulty, LevelConfig, LevelStep, ObstacleType, TargetObstacle } from '../types';
...
export class BottlePhysics {
  ...
  public setObstacle(config: LevelConfig | LevelStep | null) {
    ...
    if (type === 'toyblocks') {
      halfW = 0.45; // Top blue block with stable landing deck
      halfD = 0.45;
      rad = 0.50;
    } else if (type === 'benchpress') {
      halfW = 0.60; // Workout bench press pad and weight rack
      halfD = 0.48;
      rad = 0.56;
    } else if (type === 'cafetable') {
      halfW = 0.48; // Square cafe table
      halfD = 0.48;
      rad = 0.52;
    } else if (type === 'armchair') {
      halfW = 0.55; // Armchair / sofa cushion
      halfD = 0.50;
      rad = 0.55;
    } else if (type === 'shelf_binders') {
      halfW = 0.50; // Wall shelf with binders
      halfD = 0.40;
      rad = 0.48;
    } else if (type === 'tv') {
  ...
  public calculateLaunchParams(
    ...
    // Target spatial parameters relative to launch position
    const startX = this.state.startPosition ? this.state.startPosition.x : 0;
    const startZ = this.state.startPosition ? this.state.startPosition.z : 0;
    const startY = this.state.startPosition ? Math.max(0, this.state.startPosition.y - this.centerOfMassY) : 0;

    const targetDist = this.targetCenter.z || 2.0;
    const targetH = this.obstacle && this.obstacle.topY !== undefined ? Math.max(0, this.obstacle.topY) : Math.max(0, this.targetCenter.y || 0);
    const targetX = this.targetCenter.x || 0;

    const relDist = Math.max(0.7, targetDist - startZ);
    const relX = targetX - startX;
    const relH = Math.max(0, targetH - startY);

    // 1. Forward impulse:
    // Smooth responsive scaling from close range to far range
    const baseForward = 0.52 + clampedPower * 1.12;
    const distScale = 1.0 + Math.max(0, (relDist - 1.8) * 0.14);
    const forwardImpulse = baseForward * distScale;

    // 2. Upward launch impulse with elevation compensation:
    // For elevated obstacles, provides needed arc height
    const elevationLift = relH > 0 ? (Math.sqrt(2 * 9.2 * relH) * 0.88 + relH * 0.65) : 0;
    const upwardImpulse = 3.3 + clampedPower * 2.4 + elevationLift;

    // 3. Lateral impulse with soft deadzone & aim guidance:
    let sideImpulse = 0;
    const deadzone = 14;
    ...
    // If target has a lateral offset relative to start, guide launch appropriately:
    if (Math.abs(relX) > 0.04) {
      const targetAimAngle = Math.atan2(relX, relDist);
      ...
      if (Math.sign(userSwipeAngle) === Math.sign(targetAimAngle) || Math.abs(dragDx) < deadzone) {
        const assistAmount = Math.abs(dragDx) < deadzone ? 0.38 : 0.58;
        const desiredVx = (relX / relDist) * forwardImpulse;
