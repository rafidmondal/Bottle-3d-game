...
import { SkinItem, TargetObstacle, RoomTheme } from '../types';
import { SKINS_BOTTLES, SKINS_CAPS } from '../data/skinsData';
import { detectDeviceQuality, DeviceQualityConfig } from './deviceQuality';
...
export class SceneBuilder {
  ...
  public quality: DeviceQualityConfig;
  public roomDecorGroup!: THREE.Group;
  public classicDecorGroup!: THREE.Group;
  public backWallMesh!: THREE.Mesh;
  public leftWallMesh!: THREE.Mesh;
  public baseboardMesh!: THREE.Mesh;
  public floorMesh!: THREE.Mesh;
  public floorMat!: THREE.MeshStandardMaterial;
  public backWallMat!: THREE.MeshStandardMaterial;
  public leftWallMat!: THREE.MeshStandardMaterial;
  public baseboardMat!: THREE.MeshStandardMaterial;
  public tableMat!: THREE.MeshStandardMaterial;
  public apronMat!: THREE.MeshStandardMaterial;
  public legMat!: THREE.MeshStandardMaterial;
  public brassMat!: THREE.MeshStandardMaterial;
  public currentTheme: RoomTheme = 'kitchen';

  constructor(canvas: HTMLCanvasElement, qualityOverride?: Partial<DeviceQualityConfig>) {
    this.quality = { ...detectDeviceQuality(), ...qualityOverride };
  ...
  private buildEnvironment() {
    ...
    const ambientLight = new THREE.AmbientLight('#fff7ed', this.quality.shadowsEnabled ? 0.85 : 1.15);
    ...
    if (this.quality.shadowsEnabled) {
      ...
      sunLight.shadow.mapSize.width = this.quality.shadowMapSize;
      sunLight.shadow.mapSize.height = this.quality.shadowMapSize;
    ...
    this.floorMesh.position.set(0, -1.85, 2.5);
    this.floorMesh.receiveShadow = true;
    this.scene.add(this.floorMesh);

    // Dynamic Decor Groups for different themes (Playroom, Gym, Office, Living Room, Kitchen)
    this.roomDecorGroup = new THREE.Group();
    this.scene.add(this.roomDecorGroup);

    this.classicDecorGroup = new THREE.Group();
    this.scene.add(this.classicDecorGroup);
  ...
  private buildRoomArchitecture() {
    ...
    this.backWallMat = new THREE.MeshStandardMaterial({
      color: '#e4dacf',
      roughness: 0.85,
    });
    this.backWallMesh = new THREE.Mesh(backWallGeom, this.backWallMat);
    this.backWallMesh.position.set(0, 3.2, 7.5);
    ...
    this.leftWallMat = new THREE.MeshStandardMaterial({
      color: '#dfd4c7',
      roughness: 0.85,
    });
    this.leftWallMesh = new THREE.Mesh(leftWallGeom, this.leftWallMat);
    this.leftWallMesh.rotation.y = Math.PI / 2;
    this.leftWallMesh.position.set(-5.5, 3.2, 2.5);
    this.leftWallMesh.receiveShadow = true;
    this.scene.add(this.leftWallMesh);

    // Baseboard along back wall
    const baseboardGeom = new THREE.BoxGeometry(18, 0.28, 0.08);
    this.baseboardMat = new THREE.MeshStandardMaterial({ color: '#8b5a2b', roughness: 0.5 });
    this.baseboardMesh = new THREE.Mesh(baseboardGeom, this.baseboardMat);
    this.baseboardMesh.position.set(0, -1.71, 7.45);
    this.scene.add(this.baseboardMesh);

    // Window Frame on Left Wall (part of classic decor)
    ...
    winGroup.add(vBar);
    this.classicDecorGroup.add(winGroup);

    // Chalkboard Poster 1 (Left wall): "SMALL FLIP BIG HAPPINESS 😊"
    const chalk1 = this.createFramedChalkboardMesh(
      -2.4,
      3.2,
      7.42,
    ...
    this.classicDecorGroup.add(chalk1);

    // Chalkboard Poster 2 (Right wall): "PRACTICE PLAY IMPROVE BE A PRO 👑"
    const chalk2 = this.createFramedChalkboardMesh(
      2.4,
      3.2,
      7.42,
    ...
    this.classicDecorGroup.add(chalk2);

    // Decorative Items (enabled on medium and high quality tiers)
    if (this.quality.enableHeavyDecorations) {
      // Decorative Houseplant on Left Corner Shelf
      this.classicDecorGroup.add(this.createPottedPlantMesh(-3.8, 1.2, 4.8));
      this.classicDecorGroup.add(this.createPottedPlantMesh(3.8, 1.2, 5.2));

      // Bookshelf with "GOOD VIBES PLAY MORE" on left
      this.classicDecorGroup.add(this.createBookshelfMesh(-3.6, -0.4, 2.8));

      // Cozy Table Lamp on Right
      this.classicDecorGroup.add(this.createTableLampMesh(3.6, -0.2, 3.0));
    }
  }

  // Dynamically update room theme architecture, colors, and props
  public updateRoomTheme(theme: RoomTheme = 'kitchen') {
    this.currentTheme = theme;

    // Dispose and clear existing theme-specific room props
    while (this.roomDecorGroup.children.length > 0) {
      const child = this.roomDecorGroup.children[0];
      this.roomDecorGroup.remove(child);
      child.traverse((n) => {
    ...
    if (theme === 'playroom') {
      // 1. PLAYROOM THEME (Top-Left Screenshot: Peach/coral floor, blush walls, hanging biplanes)
      this.backWallMat.color.set('#fbeee6');
      this.leftWallMat.color.set('#f9e5db');
      this.baseboardMat.color.set('#3b82f6');
      this.floorMat.map = null;
      this.floorMat.color.set('#fba583');
      this.floorMat.needsUpdate = true;
      this.classicDecorGroup.visible = false;
      if (this.tableMat) this.tableMat.color.set('#fde68a');
      if (this.apronMat) this.apronMat.color.set('#b45309');
      if (this.brassMat) this.brassMat.color.set('#fbbf24');

      // Hanging toy biplanes from screenshot
      const blueBiplane = this.createToyBiplane('#2563eb', '#f59e0b');
      blueBiplane.position.set(-2.6, 3.4, 3.2);
      blueBiplane.rotation.set(0.15, -0.4, 0.1);
      this.roomDecorGroup.add(blueBiplane);

      const redBiplane = this.createToyBiplane('#ef4444', '#38bdf8');
      redBiplane.position.set(2.8, 3.2, 3.8);
      redBiplane.rotation.set(-0.1, 0.5, -0.15);
      this.roomDecorGroup.add(redBiplane);

      // Colorful block accents along baseboard
      const blockGeom = new THREE.BoxGeometry(0.22, 0.22, 0.22);
      const bColors = ['#ec4899', '#a855f7', '#06b6d4', '#22c55e', '#f59e0b'];
      [-2.2, -1.8, 2.0, 2.4].forEach((bx, idx) => {
        const bMat = new THREE.MeshStandardMaterial({ color: bColors[idx % bColors.length], roughness: 0.3 });
        const bMesh = new THREE.Mesh(blockGeom, bMat);
        bMesh.position.set(bx, -1.74, 5.8);
        bMesh.rotation.y = idx * 0.4;
        this.roomDecorGroup.add(bMesh);
      });

    } else if (theme === 'gym') {
      // 2. GYM THEME (Top-Right Screenshot: Solid turquoise floor, mint walls, speedbag on wall)
      this.backWallMat.color.set('#e2f7f0');
      this.leftWallMat.color.set('#d8f0e8');
      this.baseboardMat.color.set('#1e3a5f');
      this.floorMat.map = null;
      this.floorMat.color.set('#06b6d4');
      this.floorMat.needsUpdate = true;
      this.classicDecorGroup.visible = false;
      if (this.tableMat) this.tableMat.color.set('#94a3b8');
      if (this.apronMat) this.apronMat.color.set('#334155');
      if (this.brassMat) this.brassMat.color.set('#06b6d4');

      // Punching speedbag mounted to left wall
      const speedbag = this.createPunchingSpeedbag();
      speedbag.position.set(-3.2, 2.1, 2.8);
      this.roomDecorGroup.add(speedbag);

      // Contrast gym wall stripe
      const stripeGeom = new THREE.BoxGeometry(18, 0.12, 0.02);
      const stripeMat = new THREE.MeshStandardMaterial({ color: '#0891b2', roughness: 0.4 });
      const stripe = new THREE.Mesh(stripeGeom, stripeMat);
      stripe.position.set(0, 2.4, 7.44);
      this.roomDecorGroup.add(stripe);

    } else if (theme === 'office') {
      // 3. OFFICE THEME (Bottom-Left Screenshot: Sky blue walls, cool gray floor, locker & shelf)
      this.backWallMat.color.set('#cfe7f0');
      this.leftWallMat.color.set('#c2e1ed');
      this.baseboardMat.color.set('#f8fafc');
      this.floorMat.map = null;
      this.floorMat.color.set('#94a3b8');
      this.floorMat.needsUpdate = true;
      this.classicDecorGroup.visible = false;
      if (this.tableMat) this.tableMat.color.set('#fed7aa');
      if (this.apronMat) this.apronMat.color.set('#475569');
      if (this.brassMat) this.brassMat.color.set('#3b82f6');

      // Metal Wall Locker on left wall
      const locker = this.createWallLocker();
      locker.position.set(-3.2, 1.8, 2.6);
      this.roomDecorGroup.add(locker);

      // Wall Shelf with Blue Binders on right wall
      const binderShelf = this.createWallShelfWithBinders();
      binderShelf.position.set(2.4, 1.9, 3.2);
      this.roomDecorGroup.add(binderShelf);

    } else if (theme === 'livingroom') {
      // 4. LIVING ROOM THEME (Bottom-Right Screenshot: Linen off-white, mocha floor, shelf with binders & color swatch art)
      this.backWallMat.color.set('#f5f0eb');
      this.leftWallMat.color.set('#ede5dd');
      this.baseboardMat.color.set('#78716c');
      this.floorMat.map = null;
      this.floorMat.color.set('#9a7b66');
      this.floorMat.needsUpdate = true;
      this.classicDecorGroup.visible = false;
      if (this.tableMat) this.tableMat.color.set('#fed7aa');
      if (this.apronMat) this.apronMat.color.set('#78350f');
      if (this.brassMat) this.brassMat.color.set('#f59e0b');

      // Wall shelf with binders on left wall
      const binderShelf = this.createWallShelfWithBinders();
      binderShelf.position.set(-2.8, 2.1, 2.6);
      this.roomDecorGroup.add(binderShelf);

      // Color Swatch Art Poster with Lime Green Frame on right wall
      const artPoster = this.createColorSwatchArt();
      artPoster.position.set(2.6, 2.4, 3.0);
      this.roomDecorGroup.add(artPoster);

    } else {
      // 5. KITCHEN / CLASSIC THEME (Original warm cream, hardwood parquet, window & chalkboard)
      this.backWallMat.color.set('#e4dacf');
      this.leftWallMat.color.set('#dfd4c7');
      this.baseboardMat.color.set('#8b5a2b');
      this.floorMat.map = getWarmFloorTexture();
      this.floorMat.color.set('#ffffff');
      this.floorMat.needsUpdate = true;
      this.classicDecorGroup.visible = true;
      if (this.tableMat) this.tableMat.color.set('#ffffff');
      if (this.apronMat) this.apronMat.color.set('#8b5a2b');
      if (this.brassMat) this.brassMat.color.set('#f59e0b');
    }
  }

  // Framed Chalkboards
  private createFramedChalkboardMesh(
    x: number,
    y: number,
    z: number,
  ...
  private createPottedPlantMesh(x: number, y: number, z: number): THREE.Group {
  ...
  private createBookshelfMesh(x: number, y: number, z: number): THREE.Group {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      book.rotation.y = (i % 2 === 0 ? 0.08 : -0.08);
  ...
  private createTableLampMesh(x: number, y: number, z: number): THREE.Group {
  ...
  // Toy Biplane model suspended from ceiling (Playroom screenshot)
  private createToyBiplane(bodyColor = '#2563eb', wingColor = '#f59e0b'): THREE.Group {
    const plane = new THREE.Group();

    // Fuselage
    const fuseGeom = new THREE.BoxGeometry(0.55, 0.14, 0.14);
    const bodyMat = new THREE.MeshStandardMaterial({ color: bodyColor, roughness: 0.3 });
    const fuse = new THREE.Mesh(fuseGeom, bodyMat);
    plane.add(fuse);

    // Top and bottom wings
    const wingGeom = new THREE.BoxGeometry(0.20, 0.02, 0.95);
    const wingMat = new THREE.MeshStandardMaterial({ color: wingColor, roughness: 0.3 });
    const topWing = new THREE.Mesh(wingGeom, wingMat);
    topWing.position.set(0.06, 0.14, 0);
    plane.add(topWing);

    const botWing = new THREE.Mesh(wingGeom, wingMat);
    botWing.position.set(0.06, -0.08, 0);
    plane.add(botWing);

    // Wing struts
    const strutGeom = new THREE.CylinderGeometry(0.008, 0.008, 0.22, 8);
    const strutMat = new THREE.MeshStandardMaterial({ color: '#f8fafc', roughness: 0.2 });
    [-0.35, 0.35].forEach((wz) => {
      const strut = new THREE.Mesh(strutGeom, strutMat);
      strut.position.set(0.06, 0.03, wz);
      plane.add(strut);
    });

    // Propeller at nose
    const noseGeom = new THREE.ConeGeometry(0.07, 0.08, 16);
    noseGeom.rotateZ(-Math.PI / 2);
    const nose = new THREE.Mesh(noseGeom, bodyMat);
    nose.position.set(0.31, 0, 0);
    plane.add(nose);

    const propBladeGeom = new THREE.BoxGeometry(0.015, 0.26, 0.03);
    const propMat = new THREE.MeshStandardMaterial({ color: '#f1f5f9', roughness: 0.2 });
    const propBlade = new THREE.Mesh(propBladeGeom, propMat);
    propBlade.position.set(0.36, 0, 0);
    propBlade.rotation.x = 0.6;
    plane.add(propBlade);

    // Tail fin
    const finGeom = new THREE.BoxGeometry(0.12, 0.12, 0.02);
    const fin = new THREE.Mesh(finGeom, bodyMat);
    fin.position.set(-0.25, 0.10, 0);
    plane.add(fin);

    const hFinGeom = new THREE.BoxGeometry(0.10, 0.02, 0.28);
    const hFin = new THREE.Mesh(hFinGeom, wingMat);
    hFin.position.set(-0.25, 0.03, 0);
    plane.add(hFin);

    // Suspension string to ceiling
    const stringGeom = new THREE.CylinderGeometry(0.003, 0.003, 2.5, 6);
    const stringMat = new THREE.MeshBasicMaterial({ color: '#94a3b8' });
    const string = new THREE.Mesh(stringGeom, stringMat);
    string.position.set(0, 1.25, 0);
    plane.add(string);

    return plane;
  }

  // Punching speedbag with wall mounting bracket (Gym screenshot)
  private createPunchingSpeedbag(): THREE.Group {
    const bagGroup = new THREE.Group();

    // Wall mounting bracket
    const mountGeom = new THREE.BoxGeometry(0.12, 0.60, 0.70);
    const mountMat = new THREE.MeshStandardMaterial({ color: '#1e293b', roughness: 0.4 });
    const mount = new THREE.Mesh(mountGeom, mountMat);
    mount.position.set(-0.25, 0.25, 0);
    bagGroup.add(mount);

    // Circular wooden rebound swivel board
    const boardGeom = new THREE.CylinderGeometry(0.40, 0.40, 0.05, 32);
    const boardMat = new THREE.MeshStandardMaterial({ color: '#78350f', roughness: 0.3 });
    const board = new THREE.Mesh(boardGeom, boardMat);
    board.position.set(0.10, 0, 0);
    bagGroup.add(board);

    // Chrome swivel link
    const swivelGeom = new THREE.CylinderGeometry(0.02, 0.02, 0.08, 16);
    const chromeMat = new THREE.MeshStandardMaterial({ color: '#e2e8f0', metalness: 0.9, roughness: 0.1 });
    const swivel = new THREE.Mesh(swivelGeom, chromeMat);
    swivel.position.set(0.10, -0.06, 0);
    bagGroup.add(swivel);

    // Teardrop / egg speedbag (red and white sections)
    const bagGeom = new THREE.SphereGeometry(0.16, 24, 24);
    bagGeom.scale(0.85, 1.35, 0.85);
    const bagMat = new THREE.MeshStandardMaterial({ color: '#dc2626', roughness: 0.35 });
    const bag = new THREE.Mesh(bagGeom, bagMat);
    bag.position.set(0.10, -0.28, 0);
    bagGroup.add(bag);

    return bagGroup;
  }

  // Metal Wall Locker Cabinet (Office screenshot)
  private createWallLocker(): THREE.Group {
    const lockerGroup = new THREE.Group();

    // Steel locker body
    const bodyGeom = new THREE.BoxGeometry(0.45, 1.30, 0.65);
    const bodyMat = new THREE.MeshStandardMaterial({ color: '#94a3b8', roughness: 0.35, metalness: 0.2 });
    const body = new THREE.Mesh(bodyGeom, bodyMat);
    lockerGroup.add(body);

    // Chrome handle
    const handleGeom = new THREE.BoxGeometry(0.03, 0.12, 0.03);
    const chromeMat = new THREE.MeshStandardMaterial({ color: '#f1f5f9', metalness: 0.85, roughness: 0.15 });
    const handle = new THREE.Mesh(handleGeom, chromeMat);
    handle.position.set(0.24, 0.05, 0.18);
    lockerGroup.add(handle);

    // Louvers / vents
    for (let i = 0; i < 3; i++) {
      const ventGeom = new THREE.BoxGeometry(0.02, 0.015, 0.22);
      const ventMat = new THREE.MeshStandardMaterial({ color: '#475569', roughness: 0.5 });
      const vent = new THREE.Mesh(ventGeom, ventMat);
      vent.position.set(0.23, 0.45 - i * 0.04, 0.05);
  ...
  // Floating Wall Shelf with Blue Binders (Office & Living Room screenshots)
  private createWallShelfWithBinders(): THREE.Group {
    const group = new THREE.Group();

    // Wooden shelf plank
    const shelfGeom = new THREE.BoxGeometry(1.4, 0.06, 0.42);
    const shelfMat = new THREE.MeshStandardMaterial({ color: '#5c3a21', roughness: 0.4 });
    const shelf = new THREE.Mesh(shelfGeom, shelfMat);
    group.add(shelf);

    // 2 metal wall brackets
    [-0.50, 0.50].forEach((bx) => {
      const bGeom = new THREE.BoxGeometry(0.04, 0.25, 0.38);
      const bMat = new THREE.MeshStandardMaterial({ color: '#1e293b', metalness: 0.8, roughness: 0.2 });
      const b = new THREE.Mesh(bGeom, bMat);
      b.position.set(bx, -0.12, 0);
      group.add(b);
    });

    // 4 Blue Ring Binders standing on shelf
    const binderColors = ['#1d4ed8', '#2563eb', '#1e40af', '#3b82f6'];
    for (let i = 0; i < 4; i++) {
      const binderGeom = new THREE.BoxGeometry(0.09, 0.34, 0.28);
      const binderMat = new THREE.MeshStandardMaterial({ color: binderColors[i], roughness: 0.3 });
      const binder = new THREE.Mesh(binderGeom, binderMat);
      binder.position.set(-0.35 + i * 0.11, 0.20, 0);
      group.add(binder);

      // Spine label
      const labelGeom = new THREE.BoxGeometry(0.092, 0.12, 0.01);
      const labelMat = new THREE.MeshBasicMaterial({ color: '#f8fafc' });
      const label = new THREE.Mesh(labelGeom, labelMat);
      label.position.set(-0.35 + i * 0.11, 0.22, 0.141);
      group.add(label);
    }
  ...
  // Color Swatch Art Poster with Lime Green Frame (Living room screenshot)
  private createColorSwatchArt(): THREE.Group {
    const group = new THREE.Group();

    // Lime green frame
    const frameW = 1.1;
    const frameH = 1.4;
    const frameGeom = new THREE.BoxGeometry(frameW, frameH, 0.04);
    const frameMat = new THREE.MeshStandardMaterial({ color: '#84cc16', roughness: 0.3 });
    const frame = new THREE.Mesh(frameGeom, frameMat);
    group.add(frame);

    // Inner canvas
    const canvasGeom = new THREE.PlaneGeometry(frameW - 0.14, frameH - 0.14);
    const canvasMat = new THREE.MeshStandardMaterial({ color: '#fefce8', roughness: 0.6 });
    const canvas = new THREE.Mesh(canvasGeom, canvasMat);
    canvas.position.set(0, 0, 0.022);
    group.add(canvas);

    // Color swatch stripes
    const swatchColors = ['#ef4444', '#f59e0b', '#10b981', '#06b6d4', '#8b5cf6'];
    swatchColors.forEach((col, idx) => {
      const barGeom = new THREE.PlaneGeometry(0.12, 0.85);
      const barMat = new THREE.MeshBasicMaterial({ color: col });
      const bar = new THREE.Mesh(barGeom, barMat);
      bar.position.set(-0.32 + idx * 0.16, 0, 0.024);
      group.add(bar);
    });
  ...
  private buildGoldenOakTable() {
    ...
    this.apronMat = new THREE.MeshStandardMaterial({
      color: '#8b5a2b',
      roughness: 0.45,
    });
    this.legMat = new THREE.MeshStandardMaterial({ color: '#7a481f', roughness: 0.4 });
    ...
    this.tableMesh = new THREE.Mesh(longGeom, this.tableMat);
    ...
    const apronLong = new THREE.Mesh(apronGeomLong, this.apronMat);
    ...
    legPositionsLong.forEach(([lx, ly, lz]) => {
      const legMesh = new THREE.Mesh(legGeom, this.legMat);
      legMesh.position.set(lx, ly, lz);
      legMesh.castShadow = true;
      legMesh.receiveShadow = true;
      this.longTableGroup.add(legMesh);

      const ferruleMesh = new THREE.Mesh(ferruleGeom, this.brassMat);
    ...
    const shortTableMesh = new THREE.Mesh(shortGeom, this.tableMat);
    ...
    const apronShort = new THREE.Mesh(apronShortGeom, this.apronMat);
  ...
  public setTableMode(mode: 'long' | 'short' | 'none' | boolean) {
    if (mode === 'long' || mode === true) {
      if (this.longTableGroup) this.longTableGroup.visible = true;
      if (this.shortTableGroup) this.shortTableGroup.visible = false;
    } else {
      // CRITICAL USER FIX: The player's bottle MUST ALWAYS sit on a table/counter surface!
      // Never leave the bottle floating in mid-air space. Short launch table is always shown for obstacles/any mode.
      if (this.longTableGroup) this.longTableGroup.visible = false;
      if (this.shortTableGroup) this.shortTableGroup.visible = true;
    }
  }
  ...
  public buildObstacle(obs: TargetObstacle): THREE.Object3D {
    ...
    if (obs.type === 'toyblocks') {
      // 1. PLAYROOM STEPPED LEGO TOY BLOCKS (Screenshot: colorful tiered blocks with studs)
      const baseH = totalH * 0.45;
      const midH = totalH * 0.32;
      const topH = totalH * 0.23;

      // Base Yellow Block
      const baseW = 1.05;
      const baseD = 1.05;
      const baseGeom = new THREE.BoxGeometry(baseW, baseH, baseD);
      const yellowMat = new THREE.MeshStandardMaterial({ color: '#f59e0b', roughness: 0.28 });
      const baseMesh = new THREE.Mesh(baseGeom, yellowMat);
      baseMesh.position.set(0, floorY + baseH * 0.5, 0);
      ...
      const midW = 0.78;
      const midD = 0.78;
      const midGeom = new THREE.BoxGeometry(midW, midH, midD);
      const redMat = new THREE.MeshStandardMaterial({ color: '#ef4444', roughness: 0.28 });
      const midMesh = new THREE.Mesh(midGeom, redMat);
      midMesh.position.set(0, floorY + baseH + midH * 0.5, 0);
      midMesh.castShadow = true;
      midMesh.receiveShadow = true;
      group.add(midMesh);

      // Green ramp wedge on left
      const wedgeGeom = new THREE.BoxGeometry(0.28, baseH, 0.7);
      const greenMat = new THREE.MeshStandardMaterial({ color: '#22c55e', roughness: 0.3 });
      const wedge = new THREE.Mesh(wedgeGeom, greenMat);
      wedge.position.set(-baseW * 0.5 - 0.14, floorY + baseH * 0.5, 0);
      wedge.rotation.z = -0.15;
      wedge.castShadow = true;
      group.add(wedge);

      // Top Blue Landing Block
      const topW = 0.58;
      const topD = 0.58;
      const topGeom = new THREE.BoxGeometry(topW, topH, topD);
      const blueMat = new THREE.MeshStandardMaterial({ color: '#2563eb', roughness: 0.25 });
      const topMesh = new THREE.Mesh(topGeom, blueMat);
      topMesh.position.set(0, targetH - topH * 0.5, 0);
      topMesh.castShadow = true;
      topMesh.receiveShadow = true;
      group.add(topMesh);

      // Lego studs on top block
      const studGeom = new THREE.CylinderGeometry(0.04, 0.04, 0.02, 16);
      [-0.16, 0.16].forEach((sx) => {
        [-0.16, 0.16].forEach((sz) => {
          const stud = new THREE.Mesh(studGeom, blueMat);
          stud.position.set(sx, targetH + 0.01, sz);
          group.add(stud);
        });
      });

      addTopCoaster(radius * 0.85);

    } else if (obs.type === 'benchpress') {
      // 2. GYM WEIGHT BENCH PRESS (Screenshot: leather bench, barbell, chrome uprights, 45lb plates)
      const benchPadW = 0.55;
      const benchPadD = 1.25;
      const benchPadH = 0.12;
      const padY = targetH - benchPadH * 0.5;

      // Padded Black Leather Bench
      const padGeom = new THREE.BoxGeometry(benchPadW, benchPadH, benchPadD);
      const leatherMat = new THREE.MeshStandardMaterial({ color: '#18181b', roughness: 0.45 });
      const pad = new THREE.Mesh(padGeom, leatherMat);
      pad.position.set(0, padY, 0);
      pad.castShadow = true;
      pad.receiveShadow = true;
      group.add(pad);

      // Bench Steel Legs down to floor
      const legMat = new THREE.MeshStandardMaterial({ color: '#27272a', metalness: 0.7, roughness: 0.3 });
      const benchLegGeom = new THREE.BoxGeometry(0.06, padY - floorY, 0.06);
      [-0.2, 0.2].forEach((lx) => {
        [-0.45, 0.45].forEach((lz) => {
          const leg = new THREE.Mesh(benchLegGeom, legMat);
          leg.position.set(lx, floorY + (padY - floorY) * 0.5, lz);
          leg.castShadow = true;
          group.add(leg);
        });
      });

      // Chrome Uprights for Barbell (behind center)
      const uprightH = (padY - floorY) + 0.55;
      const uprightGeom = new THREE.CylinderGeometry(0.025, 0.025, uprightH, 16);
      const chromeMat = new THREE.MeshStandardMaterial({ color: '#e2e8f0', metalness: 0.9, roughness: 0.15 });
      [-0.45, 0.45].forEach((ux) => {
        const upright = new THREE.Mesh(uprightGeom, chromeMat);
        upright.position.set(ux, floorY + uprightH * 0.5, 0.35);
        upright.castShadow = true;
        group.add(upright);
      });

      // Barbell Bar
      const barGeom = new THREE.CylinderGeometry(0.018, 0.018, 1.45, 16);
      barGeom.rotateZ(Math.PI / 2);
      const barbell = new THREE.Mesh(barGeom, chromeMat);
      barbell.position.set(0, floorY + uprightH - 0.05, 0.35);
      barbell.castShadow = true;
      group.add(barbell);

      // Black 45lb Weight Plates on each side
      const plateGeom = new THREE.CylinderGeometry(0.24, 0.24, 0.05, 32);
      plateGeom.rotateZ(Math.PI / 2);
      const plateMat = new THREE.MeshStandardMaterial({ color: '#09090b', roughness: 0.4 });
      [-0.60, 0.60].forEach((px) => {
        const plate = new THREE.Mesh(plateGeom, plateMat);
        plate.position.set(px, floorY + uprightH - 0.05, 0.35);
        plate.castShadow = true;
        group.add(plate);
      });

      addTopCoaster(radius * 0.85, 0.003, -0.15);

    } else if (obs.type === 'cafetable') {
      // 3. CAFE BISTRO PEDESTAL TABLE (Screenshot: square wooden top, black metal pedestal & base)
      const topW = 0.85;
      const topD = 0.85;
      const topThick = 0.06;

      // Tabletop
      const topGeom = new THREE.BoxGeometry(topW, topThick, topD);
      const woodMat = new THREE.MeshStandardMaterial({ color: '#451a03', roughness: 0.35 });
      const tabletop = new THREE.Mesh(topGeom, woodMat);
      tabletop.position.set(0, targetH - topThick * 0.5, 0);
      tabletop.castShadow = true;
      tabletop.receiveShadow = true;
      group.add(tabletop);

      // Metal Center Pedestal Pole
      const poleH = targetH - topThick - floorY;
      const poleGeom = new THREE.CylinderGeometry(0.05, 0.05, poleH, 24);
      const poleMat = new THREE.MeshStandardMaterial({ color: '#18181b', metalness: 0.8, roughness: 0.25 });
      const pole = new THREE.Mesh(poleGeom, poleMat);
      pole.position.set(0, floorY + poleH * 0.5, 0);
      pole.castShadow = true;
      group.add(pole);

      // Heavy Square Floor Base Plate
      const baseGeom = new THREE.BoxGeometry(0.55, 0.04, 0.55);
      const basePlate = new THREE.Mesh(baseGeom, poleMat);
      basePlate.position.set(0, floorY + 0.02, 0);
      basePlate.receiveShadow = true;
      group.add(basePlate);

      addTopCoaster(radius);

    } else if (obs.type === 'armchair') {
      // 4. MID-CENTURY LIVING ROOM ARMCHAIR (Screenshot: plush purple/plum cushions, wooden frame)
      const seatW = 0.88;
      const seatD = 0.80;
      const seatH = 0.16;
      const seatY = targetH - seatH * 0.5;

      // Plum / Maroon Fabric Seat Cushion
      const seatGeom = new THREE.BoxGeometry(seatW, seatH, seatD);
      const fabricMat = new THREE.MeshStandardMaterial({ color: '#701a75', roughness: 0.65 });
      ...
      const backGeom = new THREE.BoxGeometry(seatW, 0.45, 0.16);
      const back = new THREE.Mesh(backGeom, fabricMat);
      back.position.set(0, seatY + 0.26, 0.32);
      back.rotation.x = -0.15;
      back.castShadow = true;
      group.add(back);

      // Teak Wood Armrests on sides
      const armGeom = new THREE.BoxGeometry(0.08, 0.22, seatD + 0.1);
      const armMat = new THREE.MeshStandardMaterial({ color: '#78350f', roughness: 0.35 });
      [-seatW * 0.5 - 0.04, seatW * 0.5 + 0.04].forEach((ax) => {
        const arm = new THREE.Mesh(armGeom, armMat);
        arm.position.set(ax, seatY + 0.12, 0.05);
        arm.castShadow = true;
        group.add(arm);
      });

      // 4 Angled Wooden Legs
      const legH = seatY - seatH * 0.5 - floorY;
      const legGeom = new THREE.CylinderGeometry(0.025, 0.015, legH, 16);
      [-0.35, 0.35].forEach((lx) => {
        [-0.30, 0.30].forEach((lz) => {
          const leg = new THREE.Mesh(legGeom, armMat);
    ...
    } else if (obs.type === 'shelf_binders') {
      // 5. SHELF WITH BLUE BINDERS (Screenshot: row of 4 blue binders and sturdy shelf surface)
      const shelfW = 1.15;
      const shelfD = 0.55;
      const shelfThick = 0.08;

      // Shelf Board
      const shelfGeom = new THREE.BoxGeometry(shelfW, shelfThick, shelfD);
      const shelfMat = new THREE.MeshStandardMaterial({ color: '#451a03', roughness: 0.35 });
      const shelf = new THREE.Mesh(shelfGeom, shelfMat);
      shelf.position.set(0, targetH - shelfThick * 0.5, 0);
      shelf.castShadow = true;
      shelf.receiveShadow = true;
      group.add(shelf);

      // Support pillars / brackets down to floor or wall
      const pillarGeom = new THREE.BoxGeometry(0.06, targetH - floorY, 0.06);
      const pillarMat = new THREE.MeshStandardMaterial({ color: '#1e293b', roughness: 0.4 });
      [-0.48, 0.48].forEach((px) => {
        const pillar = new THREE.Mesh(pillarGeom, pillarMat);
        pillar.position.set(px, floorY + (targetH - floorY) * 0.5, 0.2);
        pillar.castShadow = true;
        group.add(pillar);
      });

      // 4 Blue Ring Binders aligned at the back
      const binderColors = ['#1d4ed8', '#2563eb', '#1e40af', '#3b82f6'];
      for (let i = 0; i < 4; i++) {
        const bGeom = new THREE.BoxGeometry(0.085, 0.32, 0.26);
        const bMat = new THREE.MeshStandardMaterial({ color: binderColors[i], roughness: 0.3 });
        const binder = new THREE.Mesh(bGeom, bMat);
        binder.position.set(-0.35 + i * 0.10, targetH + 0.16, 0.12);
        binder.castShadow = true;
        group.add(binder);
      }

      // Landing coaster in front of binders
      addTopCoaster(radius * 0.85, 0.003, -0.10);
  ...
  public adjustCameraForTarget(targetX = 0, targetDistance = 1.8, targetHeight = 0, _theme: RoomTheme = 'kitchen') {
    ...
    const extraZ = targetDistance > 2.0 ? -(targetDistance - 2.0) * 0.32 : (1.8 - targetDistance) * 0.15;
    const extraY = targetHeight > 0.4 ? (targetHeight - 0.4) * 0.28 : 0;
    const camX = targetX * 0.22;

    this.camera.fov = baseFov;
    this.camera.position.set(camX, baseY + extraY, baseZ + extraZ);
    this.camera.lookAt(targetX * 0.45, 0.50 + Math.max(0, targetHeight) * 0.35, 1.25 + (targetDistance - 1.8) * 0.35);
    this.camera.updateProjectionMatrix();
  }

  public clearAndDisposeObstacles() {
    try {
      ...
      console.warn('[SceneBuilder] Error disposing obstacles:', err);
    }
  }
