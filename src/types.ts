...
export type RoomTheme = 'playroom' | 'gym' | 'office' | 'livingroom' | 'kitchen';
...
export interface LevelTargetStage {
  id?: string;
  title: string;
  description: string;
  roomTheme: RoomTheme;
  ...
  layoutStyle?: 'classic' | 'side-right' | 'side-left' | 'far-throw' | 'elevated' | 'moving';
  movingSpeed?: number;
  movingDistance?: number;
  wind?: number;
...
export interface LevelStep extends Partial<LevelTargetStage> {
  targetHeight: number;
  targetDistance: number;
  targetRadius: number;
  targetX?: number;
...
export interface LevelConfig {
  ...
  description: string;
  roomTheme?: RoomTheme;
  targets?: LevelTargetStage[];
  // Convenience fallbacks for active target stage / single target
  targetHeight?: number;
  targetDistance?: number;
  targetRadius?: number;
  targetX?: number;
  ...
  steps?: LevelStep[];
  layoutStyle?: 'classic' | 'side-right' | 'side-left' | 'far-throw' | 'elevated' | 'multi-step' | 'moving';
  startPosition?: { x: number; y: number; z: number };
