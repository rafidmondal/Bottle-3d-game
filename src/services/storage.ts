...
const DEFAULT_SKINS: UserSkinsState = {
  owned: ['classic', 'blue', 'capRed', 'trailNone', 'trailSparkle'],
...
// Initial levels state: level 1 is unlocked, levels 1-40 are pre-created
...
// Check if Endless Mode (Level 41+) is unlocked (unlocked after conquering Level 40)
export function isEndlessUnlocked(): boolean {
  const levels = getLevels();
  return (levels[40]?.stars || 0) >= 1 || !!levels[41]?.unlocked;
}
  ...
  try {
    ...
    if (raw) {
      ...
      if (parsed && Array.isArray(parsed.owned)) {
        if (!parsed.owned.includes('trailNone')) {
          parsed.owned.unshift('trailNone');
        }
        return parsed;
      }
    }
