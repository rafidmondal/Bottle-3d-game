...
export const AchievementsModal: React.FC<AchievementsModalProps> = ({ onBack }) => {
  ...
  // Permanent BADGES Mode Mission (400 Coins, 10s Timer, Ad Banner)
  const [sponsorWorking, setSponsorWorking] = useState<boolean>(false);
  const [sponsorTimer, setSponsorTimer] = useState<number>(0);
  const [sponsorJustClaimed, setSponsorJustClaimed] = useState<boolean>(false);

  // Countdown timer for 10s work in BADGES mode
  useEffect(() => {
    if (!sponsorWorking || sponsorTimer <= 0) return;
    const intervalId = setInterval(() => {
      setSponsorTimer((prev) => (prev > 0 ? prev - 1 : 0));
    }, 1000);
    return () => clearInterval(intervalId);
  }, [sponsorWorking, sponsorTimer]);

  // When 10s timer finishes, award +400 coins cleanly outside of state updaters
  useEffect(() => {
    if (sponsorWorking && sponsorTimer === 0) {
      setSponsorWorking(false);
      setSponsorJustClaimed(true);
      addCoins(400);
      audio.playCoin();
      setCoins(getCoins());
      setClaimToast('Claimed +400 Coins! (Worked for 10s)');
      const t1 = setTimeout(() => setClaimToast(null), 3500);
      const t2 = setTimeout(() => setSponsorJustClaimed(false), 5000);
      return () => {
        clearTimeout(t1);
        clearTimeout(t2);
      };
  ...
  const handleStartSponsorMission = () => {
    if (sponsorWorking) return;
    audio.playButton();
    // 1. Trigger the direct click ad (same click ad as name edit & bottle buy)
    triggerDirectAd();

    // 2. Start 10s timer work to earn +400 coins
    setSponsorWorking(true);
    setSponsorTimer(10);
    setSponsorJustClaimed(false);
  };
    ...
    <div className="relative w-full h-full flex flex-col justify-between p-3 sm:p-4 max-w-2xl mx-auto select-none">
      ...
      {activeTab === 'daily' ? (
        ...
        <div className="flex-1 flex flex-col justify-between overflow-hidden my-2">
          ...
          <div className="flex-1 overflow-y-auto space-y-2 pr-1 scrollbar-thin scrollbar-thumb-slate-700">
            {/* PERMANENT BADGES MODE SPECIAL MISSION (All-time permanent, 400 Coins on tap, Step 2: Worked for 10s) */}
            <div
              id="permanent-badges-mission-box"
              onClick={() => {
                if (!sponsorWorking) handleStartSponsorMission();
              }}
              className={`p-3.5 rounded-2xl border-2 transition-all cursor-pointer ${
                sponsorWorking
                  ? 'bg-gradient-to-r from-amber-950/90 via-slate-900 to-amber-900/80 border-amber-400 shadow-xl shadow-amber-500/20'
                  : sponsorJustClaimed
                  ? 'bg-gradient-to-r from-emerald-950/80 via-slate-900 to-emerald-950/80 border-emerald-400/80 shadow-md'
                  : 'bg-gradient-to-br from-amber-950/60 via-slate-900 to-slate-950 border-amber-400/80 hover:border-amber-300 shadow-lg shadow-amber-500/10'
              }`}
            >
              <div className="flex items-center justify-between gap-2 mb-2">
                <div className="flex items-center gap-2.5">
                  <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-amber-400 to-yellow-500 flex items-center justify-center text-slate-950 font-black shadow-md shrink-0">
                    <Sparkles className="w-5 h-5" />
                  </div>
                  <div>
                    <div className="text-xs font-black text-amber-300 font-['Fredoka'] flex items-center gap-1.5">
                      <span>SPECIAL BADGES MISSION</span>
                      <span className="px-1.5 py-0.2 rounded bg-amber-400 text-slate-950 font-mono font-black text-[10px]">
                        PERMANENT
                      </span>
                    </div>
                    <div className="text-[11px] text-slate-300">
                      Tap box &amp; wait 10s to earn <span className="text-amber-400 font-bold">+400 Coins</span>!
                    </div>
                  </div>
                </div>

                <div className="text-right shrink-0">
                  <span className="inline-flex items-center gap-1 px-3 py-1 rounded-xl bg-amber-400 text-slate-950 font-black text-xs font-mono shadow-md">
              ...
              {/* Sponsor Script Ad Banner (320x50 iframe running the requested snippet) */}
              <div className="my-2 flex flex-col items-center justify-center">
                <div className="w-[320px] h-[50px] max-w-full rounded-lg overflow-hidden bg-slate-950/90 border border-amber-500/30 shadow-inner flex items-center justify-center">
                  <iframe
                    title="HighRevenueFormat Ad"
                    srcDoc={`<!DOCTYPE html><html><head><meta charset="utf-8"><style>body{margin:0;padding:0;background:transparent;display:flex;align-items:center;justify-content:center;overflow:hidden;}</style></head><body><script>atOptions={'key':'145b446c792ebb559fdd28fa24c862cb','format':'iframe','height':50,'width':320,'params':{}};</script><script src="https://www.highrevenueformat.com/145b446c792ebb559fdd28fa24c862cb/invoke.js"></script></body></html>`}
                    width="320"
                    height="50"
                    className="border-0"
                    scrolling="no"
              ...
              <div className="mt-2.5">
                {sponsorWorking ? (
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between text-[11px] font-bold">
                      <span className="text-amber-300 flex items-center gap-1.5 animate-pulse">
                        <Clock className="w-3.5 h-3.5" /> Worked for 10s: in progress...
                      </span>
                      <span className="text-amber-400 font-mono font-bold">{sponsorTimer}s left</span>
                    </div>
                    <div className="w-full h-2 rounded-full bg-slate-800 overflow-hidden border border-amber-500/30">
                      <div
                        className="h-full bg-gradient-to-r from-amber-500 to-yellow-300 transition-all duration-1000 ease-linear"
                        style={{ width: `${((10 - sponsorTimer) / 10) * 100}%` }}
                      />
                    </div>
                  </div>
                ) : sponsorJustClaimed ? (
                  <div className="flex items-center justify-between p-2 rounded-xl bg-emerald-950/60 border border-emerald-500/40 text-emerald-300 text-xs font-bold">
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        handleStartSponsorMission();
                      }}
                      className="px-2.5 py-1 rounded-lg bg-amber-400 hover:bg-amber-300 text-slate-950 font-black text-[11px] active:scale-95 transition-all"
                    >
                      Tap Again
                    </button>
                  </div>
                  ...
                  <button
                    id="badges-sponsor-earn-btn"
                    onClick={(e) => {
                      e.stopPropagation();
                      handleStartSponsorMission();
                    }}
                    className="w-full py-2.5 px-3 rounded-xl bg-gradient-to-r from-amber-500 via-yellow-400 to-amber-500 hover:brightness-110 text-slate-950 font-black text-xs sm:text-sm tracking-wider shadow-md shadow-amber-500/20 active:scale-98 transition-all flex items-center justify-center gap-2"
                  >
                    <Sparkles className="w-4 h-4" />
                    <span>TAP TO EARN +400 COINS</span>
                    <span className="text-[10px] font-semibold opacity-80">(Step 2 = Worked for 10s)</span>
                  </button>
                )}
              </div>
            </div>
