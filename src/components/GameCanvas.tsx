...
export const GameCanvas: React.FC<GameCanvasProps> = ({
  ...
  const triggerCelebrationParticles = useCallback((origin: THREE.Vector3, isBig = false) => {
    const tier = sceneBuilderRef.current?.quality.tier || 'medium';
    const count = isBig ? (tier === 'low' ? 35 : tier === 'medium' ? 60 : 85) : (tier === 'low' ? 18 : tier === 'medium' ? 32 : 45);
    const colors = [
  ...
  // Helper to load target obstacle and adjust camera framing for a specific step
  const loadStepTarget = useCallback((stepIndex: number, cfg: LevelConfig) => {
    const stage = (cfg.targets && cfg.targets[stepIndex]) || (cfg.steps && cfg.steps[stepIndex]) || cfg;
    const physics = physicsRef.current;
    physics.setObstacle(stage);

    const theme = stage.roomTheme || cfg.roomTheme || 'kitchen';

    if (sceneBuilderRef.current) {
      const sb = sceneBuilderRef.current;
      sb.updateRoomTheme(theme);
      sb.clearAndDisposeObstacles();
      if (physics.obstacle && physics.obstacle.type !== 'none') {
        const obsMesh = sb.buildObstacle(physics.obstacle);
        physics.obstacle.mesh = obsMesh;
        sb.obstacleGroup.add(obsMesh);
        sb.setTableMode(theme === 'kitchen' ? 'short' : 'none');
      } else {
        sb.setTableMode(theme === 'kitchen' ? 'long' : 'none');
      }
      if (sb.targetMesh) {
        sb.targetMesh.position.set(stage.targetX || 0, 0.008, stage.targetDistance);
        sb.targetMesh.visible = stage.obstacleType === 'none' || !stage.obstacleType;
      }
      sb.adjustCameraForTarget(stage.targetX || 0, stage.targetDistance, stage.targetHeight || 0, theme);
    }
  }, []);
  ...
  const setupLevel = useCallback((lvl: number) => {
    ...
    loadStepTarget(0, cfg);

    const stage = (cfg.targets && cfg.targets[0]) || (cfg.steps && cfg.steps[0]) || cfg;
    const physics = physicsRef.current;
    const startPos = stage.startPosition
      ? new THREE.Vector3(stage.startPosition.x, physics.centerOfMassY + (stage.startPosition.y || 0), stage.startPosition.z)
      : new THREE.Vector3(0, physics.centerOfMassY, 0);

    physics.resetTo(startPos);
    if (p1BottleMeshRef.current) {
      p1BottleMeshRef.current.position.copy(startPos);
  ...
  useEffect(() => {
    ...
    const isNormalMode = !currentTrailId || currentTrailId === 'trailNone' || currentTrailId === 'none';

    let auraGroup: THREE.Group | null = null;
    let innerAuraGeom: THREE.CylinderGeometry | null = null;
    let innerAuraMat: THREE.MeshBasicMaterial | null = null;
    let innerAuraMesh: THREE.Mesh | null = null;
    let outerAuraGeom: THREE.SphereGeometry | null = null;
    let outerAuraMesh: THREE.Mesh | null = null;
    let outerAuraMat: THREE.MeshBasicMaterial | null = null;
    let auraLight: THREE.PointLight | null = null;
    ...
    if (!isNormalMode) {
      ...
      innerAuraGeom = new THREE.CylinderGeometry(0.115, 0.13, 0.42, 24, 1, true);
      innerAuraMat = new THREE.MeshBasicMaterial({
      ...
      innerAuraMesh = new THREE.Mesh(innerAuraGeom, innerAuraMat);
      auraGroup.add(innerAuraMesh);

      // Outer spherical aura halo envelope (adaptive: high and medium tiers)
      if (sb.quality.enableAuraOuterHalo) {
        outerAuraGeom = new THREE.SphereGeometry(0.22, 18, 18);
        outerAuraGeom.scale(1, 1.35, 1);
        outerAuraMat = new THREE.MeshBasicMaterial({
        ...
        outerAuraMesh = new THREE.Mesh(outerAuraGeom, outerAuraMat);
        auraGroup.add(outerAuraMesh);
      }

      // Dynamic aura point light that illuminates bottle, table, and surroundings (adaptive: medium and high tiers)
      if (sb.quality.enableAuraLight) {
        auraLight = new THREE.PointLight(auraConf.lightColor, 2.0, 3.2);
    ...
    const maxParticles = sb.quality.maxParticles;
    ...
    const loop = (time: number) => {
      ...
      if (!isPausedRef.current) {
        ...
        // Update Glowing Aura mesh and light animation (disabled in Normal Mode)
        if (auraGroup && innerAuraMesh && innerAuraMat) {
          ...
          if (currentTrailId === 'trailRainbow') {
            ...
            if (outerAuraMat) outerAuraMat.color.setHSL((hue + 0.2) % 1, 0.9, 0.6);
            if (auraLight) auraLight.color.copy(rainbowColor);
          }

          const targetInnerOpacity = isFlying ? 0.65 : 0.34;
          innerAuraMat.opacity += (targetInnerOpacity - innerAuraMat.opacity) * 0.15;
          if (outerAuraMat) outerAuraMat.opacity = innerAuraMat.opacity * 0.52;
          if (auraLight) auraLight.intensity = isFlying ? 3.8 : 1.8;

          // Ambient aura floating wisps while idle on table (only on medium/high tiers to keep low tier at 0 idle draw cost)
          if (sb.quality.tier !== 'low' && !isFlying && Math.random() < 0.22 && particlePoolRef.current.length < sb.quality.particleEmitLimit) {
        ...
        if (particlesMeshRef.current) {
          ...
          // Emit flight trail sparks when in air (only if active trail is equipped, never in Normal Mode)
          const currentTrail = getSkinsState().equippedTrail || equippedTrail;
          const isTrailActive = currentTrail && currentTrail !== 'trailNone' && currentTrail !== 'none';
          if (isTrailActive && physics.state.isInFlight && pool.length < sb.quality.particleEmitLimit) {
      ...
      // Render Three.js frame (skip when backgrounded/tab hidden to preserve mobile battery and frame budget)
  ...
  const resetBottlePosition = () => {
    if (mode === 'level') {
      const stepIdx = levelProgressRef.current.currentStepIndex;
      const stage = (levelConfig.targets && levelConfig.targets[stepIdx]) ||
                    (levelConfig.steps && levelConfig.steps[stepIdx]) || levelConfig;
      const physics = physicsRef.current;
      const startPos = stage.startPosition
        ? new THREE.Vector3(stage.startPosition.x, physics.centerOfMassY + (stage.startPosition.y || 0), stage.startPosition.z)
        : new THREE.Vector3(0, physics.centerOfMassY, 0);
  ...
  const handleFlipComplete = (res: FlipFeedback) => {
    ...
    if (mode === 'friend' || mode === 'ai') {
      ...
      const requiredFlips = levelConfig.requiredFlips || 1;
      const isMultiFlip = requiredFlips > 1;
      const progress = levelProgressRef.current;

      progress.throws += 1;
      progress.score += res.points;
      setLevelScore(progress.score);
      setLevelThrows(progress.throws);

      if (isMultiFlip) {
        if (isSuccess) {
          progress.completedFlips += 1;
          setCompletedFlips(progress.completedFlips);

          if (progress.completedFlips < requiredFlips) {
            // Step cleared! Progress to next platform target in sequence
            ...
            setFeedback({
              outcome: 'PERFECT',
              points: res.points,
              text: `FLIP ${progress.completedFlips}/${requiredFlips} CLEARED!`,
              color: '#10b981',
              flipsCompleted: progress.completedFlips,
            });

            setTimeout(() => {
              setFeedback(null);
              loadStepTarget(nextStepIdx, levelConfig);
              resetBottlePosition();
              turnStateRef.current.isSettling = false;
            }, 900);
          } else {
            // All consecutive flips completed! Level Conquered!
            const misses = progress.missesCount;
            let earnedStars = 3;
            if (misses === 1) earnedStars = 2;
            else if (misses >= 2) earnedStars = 1;

            const coinsEarned = 25 + 10 * earnedStars + (requiredFlips - 1) * 15;
            recordLevelSuccess(currentLevel, progress.score, earnedStars);
          ...
          // Missed flip
          progress.missesCount += 1;
          setMissesCount(progress.missesCount);

          if (progress.missesCount >= 3) {
            // Exhausted 3 misses: Reset back to step 0
            setTimeout(() => {
              setFeedback(null);
              progress.currentStepIndex = 0;
              progress.completedFlips = 0;
              progress.missesCount = 0;
              progress.score = 0;
              progress.throws = 0;
              setCurrentStepIndex(0);
              setCompletedFlips(0);
              ...
              setLevelScore(0);
              setLevelThrows(0);
              loadStepTarget(0, levelConfig);
              resetBottlePosition();
              turnStateRef.current.isSettling = false;
            }, 1000);
          } else {
            // Missed, but attempts remain: retry current step
            setTimeout(() => {
              setFeedback(null);
              resetBottlePosition();
              turnStateRef.current.isSettling = false;
        ...
        // Classic Single-Flip Level
        const nextScore = progress.score;
        const nextThrows = progress.throws;

        const req = levelConfig.starsScoreRequirement;
        let earnedStars = 0;
        if (nextScore >= req[2] || isPerfect) earnedStars = 3;
        else if (nextScore >= req[1] || isEdge) earnedStars = 2;
        else if (nextScore >= req[0] || isSuccess) earnedStars = 1;

        if (isSuccess) {
          setTimeout(() => {
            setFeedback(null);
            const finalStars = Math.max(1, earnedStars);
            const coinsEarned = 20 + 10 * finalStars;
            recordLevelSuccess(currentLevel, nextScore, finalStars);
            setLevelComplete({
              show: true,
              stars: finalStars,
              coins: coinsEarned,
        ...
        } else if (nextThrows >= 3 && !isSuccess) {
          setTimeout(() => {
            setFeedback(null);
            progress.score = 0;
            progress.throws = 0;
  ...
  return (
    ...
    >
      ...
      {mode === 'level' && (() => {
        const activeStage = (levelConfig.targets && levelConfig.targets[currentStepIndex]) ||
                            (levelConfig.steps && levelConfig.steps[currentStepIndex]) || null;
        const totalFlips = levelConfig.requiredFlips || 1;
        const isMultiFlip = totalFlips > 1;
        const activeTheme = activeStage?.roomTheme || levelConfig.roomTheme || 'kitchen';
        const activeStyle = activeStage?.layoutStyle || levelConfig.layoutStyle;

        return (
          <div className="absolute top-16 left-1/2 -translate-x-1/2 w-[94%] max-w-md px-3.5 py-2 rounded-2xl bg-slate-900/85 border border-white/10 text-white shadow-xl backdrop-blur-xl pointer-events-none z-10 flex items-center justify-between">
            <div className="flex items-center gap-2 overflow-hidden">
              ...
              <div className="truncate">
                <div className="text-[11px] font-bold text-slate-200 truncate flex items-center gap-1.5 flex-wrap">
                  <span className="truncate">{activeStage?.title || levelConfig.title}</span>
                  {isMultiFlip && (
                    <span className="px-1.5 py-0.5 rounded bg-sky-500/25 text-sky-300 text-[8px] font-mono border border-sky-400/30 shrink-0">
                      Step {currentStepIndex + 1}/{totalFlips}
                    </span>
                  )}
                  {activeTheme !== 'kitchen' && (
                    <span className="px-1.5 py-0.5 rounded bg-emerald-500/25 text-emerald-300 text-[8px] border border-emerald-400/30 capitalize shrink-0">
                      {activeTheme}
                    </span>
                  )}
                  {activeStyle === 'side-left' && (
                    <span className="px-1.5 py-0.5 rounded bg-amber-500/25 text-amber-300 text-[8px] border border-amber-400/30 shrink-0">
                      Left
                    </span>
                  )}
                  {activeStyle === 'side-right' && (
                    <span className="px-1.5 py-0.5 rounded bg-amber-500/25 text-amber-300 text-[8px] border border-amber-400/30 shrink-0">
                      Right
                    </span>
                  )}
                  {activeStyle === 'far-throw' && (
                    <span className="px-1.5 py-0.5 rounded bg-purple-500/25 text-purple-300 text-[8px] border border-purple-400/30 shrink-0">
                      Long Throw
                    </span>
                  )}
                </div>
                <div className="text-[9px] text-slate-400 truncate">
                  {activeStage?.description || levelConfig.description}
                </div>
              </div>
            </div>
            <div className="flex items-center gap-2 shrink-0 pl-2 border-l border-white/10 text-xs font-mono font-bold">
              {isMultiFlip ? (
                <div className="flex flex-col items-end leading-tight">
                  <span className="text-amber-400 text-[11px]">{completedFlips}/{totalFlips} Flips</span>
                  <span className="text-rose-400 text-[9px]">Misses: {missesCount}/3</span>
                </div>
              ) : (
                <>
