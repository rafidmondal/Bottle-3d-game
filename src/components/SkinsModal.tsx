...
import { ArrowLeft, Coins, Star, Check, Lock, Sparkles, Eye, RotateCw, Tv, Pin } from 'lucide-react';
...
const BottleShowcaseStage: React.FC<ShowcaseProps> = ({
  ...
  useEffect(() => {
    ...
    // Dynamic Trail Particles Ribbon Preview and Glowing Aura Halo (disabled in Normal Mode)
    if (trailId && trailId !== 'trailNone' && trailId !== 'none') {
      const pCount = 72;
...
const BottleCardThumbnail: React.FC<BottleThumbnailProps> = ({
  ...
  if (category === 'trails') {
    // Normal Mode: Pristine bottle with ZERO glow, NO light aura, NO sparkle rings
    if (itemId === 'trailNone') {
      return (
        <svg viewBox="0 0 100 100" className="w-16 h-16 drop-shadow-md">
          {/* Neutral Base Mat */}
          <ellipse cx="50" cy="78" rx="34" ry="9" fill="#0f172a" stroke="#334155" strokeWidth="1.2" strokeDasharray="3 2" />

          {/* Clean Bottle Silhouette - Zero Glow */}
          <path
            d="M 40 76 L 40 46 C 40 40 45 36 47 32 L 53 32 C 55 36 60 40 60 46 L 60 76 Z"
            fill="#1e293b"
            stroke="#38bdf8"
            strokeWidth="1.8"
          />
          {/* Water level */}
          <path
            d="M 41 75 L 41 56 Q 50 54 59 56 L 59 75 Z"
            fill="#0284c7"
            fillOpacity="0.6"
          />
          {/* Specular bottle glint */}
          <line x1="43" y1="48" x2="43" y2="72" stroke="#ffffff" strokeWidth="1.2" strokeOpacity="0.6" strokeLinecap="round" />
          {/* Cap */}
          <rect x="47" y="26" width="6" height="6" rx="1" fill="#ef4444" stroke="#ffffff" strokeWidth="0.8" />

          {/* NO GLOW Badge */}
          <rect x="22" y="8" width="56" height="15" rx="7.5" fill="#0f172a" stroke="#475569" strokeWidth="1" />
          <text x="50" y="19" fill="#94a3b8" fontSize="8" fontWeight="800" textAnchor="middle" letterSpacing="0.5">
            NO GLOW
          </text>
        </svg>
      );
            ...
            return (
              ...
              >
                ...
                <div className="w-full h-24 rounded-xl bg-gradient-to-b from-slate-950/80 to-slate-900/90 flex items-center justify-center relative overflow-hidden border border-slate-800/80 group">
                  ...
                  {item.id === 'trailNone' ? (
                    <div className="absolute top-1.5 left-1.5 px-1.5 py-0.5 rounded-md bg-gradient-to-r from-emerald-500 to-teal-500 text-slate-950 text-[9px] font-black tracking-wider shadow-md flex items-center gap-0.5 border border-emerald-300">
                      <Pin className="w-2.5 h-2.5" /> PINNED • FREE
                    </div>
                  ) : item.unlockType === 'ad' ? (
